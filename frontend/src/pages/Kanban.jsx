...
function Kanban() {
  ...
  const columns = [
    { id: 'PENDING', title: 'Aguardando', icon: Clock, color: 'text-amber-400', bg: 'bg-amber-400/10' },
    { id: 'ADJUSTMENT', title: 'Em Ajuste', icon: AlertCircle, color: 'text-rose-400', bg: 'bg-rose-400/10' },
    { id: 'APPROVED', title: 'Aprovados', icon: CheckCircle2, color: 'text-cyan-400', bg: 'bg-cyan-400/10' },
  ]

  const getFilteredPosts = (status) => {
  ...
  return (
    <div className="h-[calc(100vh-10rem)] flex flex-col">
      <div className="flex justify-between items-end mb-8 border-b border-[#1E293B] pb-6 shrink-0">
        <div>
           <h1 className="text-3xl font-black tracking-tight text-white mb-1 flex items-center gap-3">
             <KanbanIcon className="text-cyan-400" /> Fluxo de Produção
           </h1>
        ...
        <div className="flex gap-4">
           <div className="flex items-center gap-2 bg-[#0B1221] px-4 py-2 rounded-xl border border-slate-800">
      ...
      <div className="flex-1 flex gap-6 overflow-x-auto pb-4 custom-scrollbar">
        {columns.map(col => (
          <div key={col.id} className="min-w-[320px] w-full flex flex-col bg-[#0B1221]/40 rounded-[2rem] border border-slate-800/50 p-4">
            
            {/* Column Header */}
            <div className={`flex items-center justify-between px-3 py-4 mb-4 rounded-2xl ${col.bg} border border-${col.color.split('-')[1]}-500/20`}>
            ...
            <div className="flex-1 space-y-4 overflow-y-auto pr-1">
               {getFilteredPosts(col.id).map(post => (
                 <div key={post.id} className="group bg-[#0B1221] border border-slate-800/80 p-4 rounded-2xl hover:border-cyan-500/50 hover:shadow-[0_0_20px_rgba(34,211,238,0.05)] transition-all cursor-grab active:cursor-grabbing">
                    ...
                    <div className="flex items-center justify-between pt-1">
                       <div className="flex -space-x-2">
                          <div className="w-6 h-6 rounded-full bg-slate-800 border-2 border-[#0B1221] flex items-center justify-center text-[10px] font-bold text-cyan-400 ring-1 ring-cyan-500/20">
